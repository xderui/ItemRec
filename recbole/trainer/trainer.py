...
class Trainer(AbstractTrainer):
    ...
    @torch.no_grad()
    def evaluate_CF(
        self, eval_data, load_best_model=True, model_file=None, show_progress=False, k=10, k_item=5, M=50
    ):
        r"""Evaluate the model based on the eval data.
        ...
        # get mutual scores between users and items 
        for batch_idx, batched_data in enumerate(iter_data):

            interaction, history_record, positive_u, positive_i = batched_data
            ...
            scores_tensor.append(scores_sorted[0,:M].reshape(1,-1))
            scores_tensor_user.append(idx_sorted[0,:M].reshape(1,-1))


            positive_i_dict[batch_idx] = positive_i.tolist()
            history_record_dict[batch_idx] = history_record

        scores_tensor = torch.cat(scores_tensor,dim=0).cpu()
        scores_tensor_user = torch.cat(scores_tensor_user,dim=0).cpu().numpy()
        ...
        for uid in range(scores_tensor.shape[0]):
            for idx in range(M):
                ...
                # rank_ = idx + 1/scores_tensor[uid,idx]
                rank_ = idx + (scores_tensor_item_max[item_id]-scores_tensor[uid,idx]) / (scores_tensor_item_max[item_id]-scores_tensor_item_min[item_id])
                scores_tensor_item_rank[item_id].append(rank_)
        


        # sort users' rank before re-ranking
        for item_id in scores_tensor_item.keys():
            ...
            scores_tensor_item_scores[item_id] = -1*np.sort(-1*scores_tensor_item_scores[item_id])
        ...
        # select the top-k items while ensuring r(item,user) < k_item
        for uid in user_item_rank.keys():
            ...
            for item_id in list(user_item_rank[uid].keys())[:10]:
                if user_item_rank[uid][item_id] < k_item:
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            self.eval_collector.eval_batch_collect_CF(
                scores, interaction, positive_u, positive_i, topk_idx[batch_idx].reshape(1,-1)
            )
        ...
        self.eval_collector = Collector(self.config)
