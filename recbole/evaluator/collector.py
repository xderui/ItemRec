...
class Collector(object):
    ...
    def __init__(self, config):
        ...
        self.history_item = defaultdict(list)
        self.itemset = set()
    ...
    def eval_batch_collect(
        ...
        if self.register.need("rec.topk"):
            ...
            pos_matrix = torch.zeros_like(scores_tensor, dtype=torch.int)  # [batch_size, item_num]
    ...
    def eval_batch_collect_CF(
        ...
        Args:
            ...
            positive_u(Torch.Tensor): the row index of positive items for each user.
            positive_i(Torch.Tensor): the positive item id for each user.
            topk_idx(Torch.tensor): the recommendation results for each user. with the shape of `(N, k)`
        """
        if self.register.need("rec.items"):
        ...
        if self.register.need("rec.topk"):
            ...
            pos_idx = torch.gather(pos_matrix, dim=1, index=topk_idx)
            result = torch.cat((pos_idx, pos_len_list), dim=1)
            self.itemset = self.itemset | set(torch.unique(topk_idx).tolist())


            try:
                valid_cover_set = self.data_struct.get('rec.valid_cover')
                if valid_cover_set == None:
                    valid_cover_set = set()
            except:
                self.data_struct.set('rec.valid_cover',set())
                valid_cover_set = set()
            ...
            for u in range(topk_idx.shape[0]):
                for i in range(topk_idx.shape[1]):
                    if topk_idx[u,i] in positive_i:
                        valid_cover_set.add(topk_idx[u,i].item())


            self.data_struct.set('rec.valid_cover',valid_cover_set)
            self.data_struct.update_tensor("rec.topk", result)
            self.data_struct.set('rec.coverage',len(self.itemset) / scores_tensor.shape[1])
            
            self.data_struct.update_tensor("rec.final_pos_idx", topk_idx)
           

            self.data_struct.set('rec.itemnum',scores_tensor.shape[1])
