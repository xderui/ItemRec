...
class Valid_Coverage(TopkMetric):

    metric_need = ['rec.valid_cover','rec.itemnum']

    def __init__(self, config):
        super().__init__(config)
        self.valid_cover = None
        self.itemnum = 0

    def calculate_metric(self, dataobject):
        pos_index, pos_len = self.used_info(dataobject)
        self.valid_cover = dataobject.get('rec.valid_cover')
        self.itemnum = dataobject.get('rec.itemnum')
        result = self.metric_info(pos_index)
        metric_dict = self.topk_result("valid_coverage", result)
        return metric_dict

    def metric_info(self, pos_index):
        return np.tile(len(self.valid_cover)/self.itemnum,pos_index.shape)

class Coverage(TopkMetric):

    metric_need = ["rec.coverage"]
    ...
    def calculate_metric(self, dataobject):
        pos_index, _ = self.used_info(dataobject)
        print(dataobject)
        self.cover = dataobject.get('rec.coverage')
        result = self.metric_info(pos_index)
        metric_dict = self.topk_result("coverage", result)
        
        return metric_dict

    def metric_info(self, pos_index):
        cover_ = self.cover
        return np.tile(cover_,pos_index.shape)

class GiniIndex(TopkMetric):
    metric_need = ["rec.final_pos_idx","rec.itemnum"]

    def __init__(self,config):
        super().__init__(config)
        self.final_pos_idx = None
        self.itemnum = 0

    def calculate_metric(self, dataobject):
        pos_index, _ = self.used_info(dataobject)
        self.final_pos_idx = dataobject.get('rec.final_pos_idx')
        self.itemnum = dataobject.get('rec.itemnum')
        result = self.metric_info(pos_index)
        metric_dict = self.topk_result("gini_index", result)
        
        return metric_dict

    def metric_info(self, pos_index):
        pos_idx = self.final_pos_idx
        item_num = self.itemnum

        cnt = [0 for i in range(item_num)]
        row, col = pos_idx.shape
        for i in range(row):
            for j in range(col):
                cnt[pos_idx[i,j]] += 1
        ...
        return np.tile(giny,pos_index.shape)


class Entropy(TopkMetric):
    metric_need = ["rec.final_pos_idx","rec.itemnum"]
    ...
    def calculate_metric(self, dataobject):
        ...
        self.itemnum = dataobject.get('rec.itemnum')
        result = self.metric_info(pos_index)
        metric_dict = self.topk_result("entropy", result)
    ...
    def metric_info(self, pos_index):
        ...
        return np.tile(entropy,pos_index.shape)
